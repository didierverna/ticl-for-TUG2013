...
(let ((magick (gensym)))
  (defun open-environment (stream env-name)
...
(defun handle-environment (stream char)
  (declare (ignore char))
  (destructuring-bind (begin-or-end env-name)
      (read-delimited-list #\} stream t)
    (ecase (intern (symbol-name begin-or-end) :keyword)
      (:begin (open-environment stream env-name))
      (:end (close-environment env-name)))))

(defun make-user-readtable ()
  (let ((readtable (copy-readtable nil)))
    (set-macro-character #\{ #'handle-environment nil readtable)
